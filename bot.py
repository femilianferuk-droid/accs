import asyncio
import logging
import os
from datetime import datetime

from aiogram import Bot, Dispatcher, types, F
from aiogram.filters import Command, StateFilter
from aiogram.fsm.context import FSMContext
from aiogram.fsm.state import StatesGroup, State
from aiogram.fsm.storage.memory import MemoryStorage
from aiogram.types import (
    Message, CallbackQuery, InlineKeyboardMarkup, InlineKeyboardButton,
    ReplyKeyboardMarkup, KeyboardButton
)
from aiogram.enums import ParseMode
from aiogram.client.default import DefaultBotProperties
from aiogram.utils.keyboard import InlineKeyboardBuilder

from telethon import TelegramClient
from telethon.tl.functions.messages import GetHistoryRequest
from telethon.errors import SessionPasswordNeededError

import asyncpg
import re
import aiohttp

# --- НАСТРОЙКИ ---
TOKEN = os.getenv("BOT_TOKEN")
CRYPTO_BOT_TOKEN = os.getenv("CRYPTO_BOT_TOKEN")
DATABASE_URL = os.getenv("DATABASE_URL")
ADMIN_IDS = [7973988177]

# LZT.Market настройки
LZT_MERCHANT_ID = 2422
LZT_API_KEY = "59db37bed744870690cc01af8909e3f93f251b0c4f797c552067bf551a525557"
LZT_API_URL = "https://api.lzt.market"

API_ID = 32480523
API_HASH = "147839735c9fa4e83451209e9b55cfc5"

logging.basicConfig(level=logging.INFO)

bot = Bot(token=TOKEN, default=DefaultBotProperties(parse_mode=ParseMode.HTML))
storage = MemoryStorage()
dp = Dispatcher(storage=storage)

# --- ПУЛ ПОСТГРЕС ---
pool = None

async def get_pool():
    global pool
    if pool is None:
        pool = await asyncpg.create_pool(DATABASE_URL)
    return pool

# --- СОСТОЯНИЯ FSM ---
class AddAccountStates(StatesGroup):
    waiting_for_country = State()
    waiting_for_price = State()
    waiting_for_phone = State()
    waiting_for_twofa = State()
    waiting_for_media = State()

class PaymentSBP(StatesGroup):
    waiting_for_screenshot = State()

class BroadcastStates(StatesGroup):
    waiting_for_message = State()

class ChangeRequisites(StatesGroup):
    waiting_for_new_value = State()

class SearchUser(StatesGroup):
    waiting_for_query = State()

class EditAccount(StatesGroup):
    waiting_for_field = State()
    waiting_for_value = State()

# --- ПРЕМИУМ ЭМОДЗИ ID ---
EMOJI_SETTINGS = "5870982283724328568"
EMOJI_PROFILE = "5870994129244131212"
EMOJI_PEOPLE = "5870772616305839506"
EMOJI_USER_CHECK = "5891207662678317861"
EMOJI_USER_CROSS = "5893192487324880883"
EMOJI_FILE = "5870528606328852614"
EMOJI_SMILE = "5870764288364252592"
EMOJI_GRAPH_UP = "5870930636742595124"
EMOJI_STATS = "5870921681735781843"
EMOJI_HOME = "5873147866364514353"
EMOJI_LOCK_CLOSED = "6037249452824072506"
EMOJI_LOCK_OPEN = "6037496202990194718"
EMOJI_MEGAPHONE = "6039422865189638057"
EMOJI_CHECK = "5870633910337015697"
EMOJI_CROSS = "5870657884844462243"
EMOJI_PENCIL = "5870676941614354370"
EMOJI_TRASH = "5870875489362513438"
EMOJI_CLIP = "6039451237743595514"
EMOJI_LINK = "5769289093221454192"
EMOJI_INFO = "6028435952299413210"
EMOJI_BOT = "6030400221232501136"
EMOJI_EYE = "6037397706505195857"
EMOJI_SEND = "5963103826075456248"
EMOJI_DOWNLOAD = "6039802767931871481"
EMOJI_BELL = "6039486778597970865"
EMOJI_GIFT = "6032644646587338669"
EMOJI_CLOCK = "5983150113483134607"
EMOJI_HURRAY = "6041731551845159060"
EMOJI_WRITE = "5870753782874246579"
EMOJI_MEDIA = "6035128606563241721"
EMOJI_GEO = "6042011682497106307"
EMOJI_WALLET = "5769126056262898415"
EMOJI_BOX = "5884479287171485878"
EMOJI_CRYPTO_BOT = "5260752406890711732"
EMOJI_CALENDAR = "5890937706803894250"
EMOJI_TAG = "5886285355279193209"
EMOJI_MONEY = "5904462880941545555"
EMOJI_SEND_MONEY = "5890848474563352982"
EMOJI_RECV_MONEY = "5879814368572478751"
EMOJI_CODE = "5940433880585605708"
EMOJI_LOADING = "5345906554510012647"
EMOJI_BACK = "5370941091573721825"
EMOJI_PHONE = "5373141891321699086"
EMOJI_BANK = "5370810157871667232"
EMOJI_KEY = "5471984997361523302"
EMOJI_SEARCH = "5370393368420557134"
EMOJI_LIST = "5373141891321699086"
EMOJI_MARKET = "6032644646587338669"

# --- БАЗА ДАННЫХ POSTGRESQL ---
async def init_db():
    p = await get_pool()
    async with p.acquire() as conn:
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS users (
                user_id BIGINT PRIMARY KEY,
                username TEXT,
                first_name TEXT,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS accounts (
                id SERIAL PRIMARY KEY,
                country TEXT,
                price INTEGER,
                phone TEXT,
                twofa TEXT,
                media_file_id TEXT,
                media_type TEXT,
                description TEXT,
                status TEXT DEFAULT 'available',
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS purchases (
                id SERIAL PRIMARY KEY,
                user_id BIGINT,
                account_id INTEGER,
                payment_method TEXT,
                amount INTEGER,
                status TEXT,
                lzt_invoice_id TEXT,
                lzt_payment_id TEXT,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS requisites (
                key TEXT PRIMARY KEY,
                value TEXT
            )
        """)
        await conn.execute("""
            INSERT INTO requisites (key, value) VALUES ('sbp_phone', '')
            ON CONFLICT (key) DO NOTHING
        """)
        await conn.execute("""
            INSERT INTO requisites (key, value) VALUES ('sbp_bank', '')
            ON CONFLICT (key) DO NOTHING
        """)
        await conn.execute("""
            INSERT INTO requisites (key, value) VALUES ('sbp_fio', '')
            ON CONFLICT (key) DO NOTHING
        """)

# --- LZT.MARKET ФУНКЦИИ ---
async def create_lzt_invoice(amount: int, payment_id: str, comment: str, user_id: int) -> dict:
    """Создаёт счёт в LZT.Market"""
    headers = {
        "Authorization": f"Bearer {LZT_API_KEY}",
        "Content-Type": "application/json"
    }
    
    data = {
        "currency": "rub",
        "amount": float(amount),
        "payment_id": payment_id,
        "comment": comment,
        "merchant_id": LZT_MERCHANT_ID,
        "required_telegram_id": user_id,
        "lifetime": 3600
    }
    
    async with aiohttp.ClientSession() as session:
        async with session.post(f"{LZT_API_URL}/invoice", headers=headers, json=data) as resp:
            result = await resp.json()
            return result

async def check_lzt_invoice(invoice_id: int) -> dict:
    """Проверяет статус счёта в LZT.Market"""
    headers = {
        "Authorization": f"Bearer {LZT_API_KEY}",
        "Content-Type": "application/json"
    }
    
    async with aiohttp.ClientSession() as session:
        async with session.get(f"{LZT_API_URL}/invoice/{invoice_id}", headers=headers) as resp:
            result = await resp.json()
            return result

# --- TELEGRAM КЛИЕНТ ДЛЯ ПОЛУЧЕНИЯ КОДА ---
async def get_code_from_telegram(phone: str, twofa: str = None) -> str:
    """Подключается к аккаунту и получает код из последнего чата"""
    try:
        session_file = f"sessions/{phone.replace('+', '')}"
        client = TelegramClient(session_file, API_ID, API_HASH)
        await client.connect()
        
        if not await client.is_user_authorized():
            try:
                sent_code = await client.send_code_request(phone)
                logging.info(f"Code sent to {phone}")
                
                # Ждём код
                await asyncio.sleep(5)
                
                # Получаем диалоги
                dialogs = await client.get_dialogs(limit=20)
                
                for dialog in dialogs:
                    if dialog.name in ["Telegram", "Telegram Verified"] or "telegram" in dialog.name.lower():
                        messages = await client.get_messages(dialog.id, limit=10)
                        for msg in messages:
                            if msg.text:
                                # Ищем код (5-6 цифр)
                                match = re.search(r'\b(\d{5,6})\b', msg.text)
                                if match:
                                    code = match.group(1)
                                    logging.info(f"Found code: {code}")
                                    await client.disconnect()
                                    return code
                
                # Если не нашли в Telegram, ищем в других чатах
                for dialog in dialogs[:10]:
                    messages = await client.get_messages(dialog.id, limit=10)
                    for msg in messages:
                        if msg.text:
                            match = re.search(r'\b(\d{5,6})\b', msg.text)
                            if match:
                                code = match.group(1)
                                logging.info(f"Found code in {dialog.name}: {code}")
                                await client.disconnect()
                                return code
                                
            except Exception as e:
                logging.error(f"Error getting code: {e}")
                await client.disconnect()
                return None
        
        await client.disconnect()
        return None
    except Exception as e:
        logging.error(f"Telethon error: {e}")
        return None

async def login_and_get_code(phone: str, twofa: str = None) -> str:
    """Авторизуется и получает код"""
    try:
        session_file = f"sessions/{phone.replace('+', '')}"
        client = TelegramClient(session_file, API_ID, API_HASH)
        await client.connect()
        
        if await client.is_user_authorized():
            await client.disconnect()
            return "already_authorized"
        
        # Отправляем запрос кода
        await client.send_code_request(phone)
        logging.info(f"Code request sent to {phone}")
        
        # Ждём доставки кода
        await asyncio.sleep(8)
        
        # Проверяем новые сообщения
        dialogs = await client.get_dialogs(limit=30)
        
        for dialog in dialogs:
            try:
                messages = await client.get_messages(dialog.id, limit=15)
                for msg in messages:
                    if msg.text and msg.date:
                        # Проверяем что сообщение не старше 5 минут
                        if (datetime.now().timestamp() - msg.date.timestamp()) < 300:
                            match = re.search(r'\b(\d{5,6})\b', msg.text)
                            if match:
                                code = match.group(1)
                                logging.info(f"Found code: {code} in {dialog.name}")
                                await client.disconnect()
                                return code
            except:
                continue
        
        await client.disconnect()
        return None
    except Exception as e:
        logging.error(f"Login error: {e}")
        return None

async def get_media_file_id(message: Message):
    """Получает file_id из сообщения с медиа"""
    if message.photo:
        return message.photo[-1].file_id, "photo"
    elif message.video:
        return message.video.file_id, "video"
    elif message.animation:
        return message.animation.file_id, "animation"
    return None, None

async def complete_purchase(purchase_id: int, acc_id: int, user_id: int, callback: CallbackQuery = None):
    """Завершает покупку: обновляет статусы и отправляет данные пользователю"""
    p = await get_pool()
    async with p.acquire() as conn:
        await conn.execute("UPDATE purchases SET status = 'completed' WHERE id = $1", purchase_id)
        await conn.execute("UPDATE accounts SET status = 'sold' WHERE id = $1", acc_id)
        
        acc_data = await conn.fetchrow("SELECT phone, twofa FROM accounts WHERE id = $1", acc_id)
        
        if acc_data:
            phone, twofa = acc_data
            
            # Получаем код при выдаче
            code = await login_and_get_code(phone, twofa)
            
            text = f'<b><tg-emoji emoji-id="{EMOJI_CHECK}">✅</tg-emoji> Оплата получена!</b>\n\n'
            text += f'<tg-emoji emoji-id="{EMOJI_PHONE}">📱</tg-emoji> Номер: <code>{phone}</code>\n'
            
            if code and code != "already_authorized":
                text += f'<tg-emoji emoji-id="{EMOJI_CODE}">🔨</tg-emoji> Код: <code>{code}</code>\n'
            elif code == "already_authorized":
                text += f'<tg-emoji emoji-id="{EMOJI_INFO}">ℹ</tg-emoji> Аккаунт уже авторизован\n'
            else:
                text += f'<tg-emoji emoji-id="{EMOJI_CROSS}">❌</tg-emoji> Не удалось получить код автоматически\n'
            
            if twofa:
                text += f'<tg-emoji emoji-id="{EMOJI_LOCK_CLOSED}">🔒</tg-emoji> 2FA: <code>{twofa}</code>\n'
            
            try:
                await bot.send_message(
                    user_id,
                    f'<b><tg-emoji emoji-id="{EMOJI_GIFT}">🎁</tg-emoji> Ваш аккаунт готов!</b>\n\n'
                    f'<tg-emoji emoji-id="{EMOJI_PHONE}">📱</tg-emoji> Номер: <code>{phone}</code>',
                    reply_markup=get_code_keyboard(acc_id)
                )
            except Exception as e:
                logging.error(f"Failed to send to user {user_id}: {e}")
            
            if callback:
                await callback.message.edit_text(text, reply_markup=get_main_keyboard())

# --- КЛАВИАТУРЫ СО СТИЛЯМИ ---
def get_main_keyboard():
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="Купить аккаунт", icon_custom_emoji_id=EMOJI_WALLET)],
            [KeyboardButton(text="Профиль", icon_custom_emoji_id=EMOJI_PROFILE)],
            [KeyboardButton(text="Поддержка", icon_custom_emoji_id=EMOJI_INFO)]
        ],
        resize_keyboard=True
    )

def get_admin_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="Статистика", callback_data="admin_stats", style='primary', icon_custom_emoji_id=EMOJI_STATS),
        InlineKeyboardButton(text="Рассылка", callback_data="admin_broadcast", style='primary', icon_custom_emoji_id=EMOJI_MEGAPHONE)
    )
    builder.row(
        InlineKeyboardButton(text="Добавить аккаунт", callback_data="admin_add_acc", style='success', icon_custom_emoji_id=EMOJI_BOX),
        InlineKeyboardButton(text="Управление аккаунтами", callback_data="admin_manage_accs", style='primary', icon_custom_emoji_id=EMOJI_LIST)
    )
    builder.row(
        InlineKeyboardButton(text="Поиск пользователя", callback_data="admin_search_user", style='primary', icon_custom_emoji_id=EMOJI_SEARCH),
        InlineKeyboardButton(text="Реквизиты СБП", callback_data="admin_change_req", style='primary', icon_custom_emoji_id=EMOJI_PENCIL)
    )
    builder.row(
        InlineKeyboardButton(text="Экспорт БД", callback_data="admin_export_db", style='default', icon_custom_emoji_id=EMOJI_DOWNLOAD),
        InlineKeyboardButton(text="Закрыть", callback_data="close_panel", style='danger', icon_custom_emoji_id=EMOJI_CROSS)
    )
    return builder.as_markup()

def get_back_button(callback_data: str = "back_to_admin"):
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Назад", callback_data=callback_data, style='default', icon_custom_emoji_id=EMOJI_BACK))
    return builder.as_markup()

def get_cancel_add_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Отмена", callback_data="cancel_add", style='danger', icon_custom_emoji_id=EMOJI_CROSS))
    return builder.as_markup()

def get_skip_media_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="Пропустить", callback_data="skip_media", style='primary', icon_custom_emoji_id=EMOJI_SEND),
        InlineKeyboardButton(text="Отмена", callback_data="cancel_add", style='danger', icon_custom_emoji_id=EMOJI_CROSS)
    )
    return builder.as_markup()

def get_payment_method_keyboard(account_id: int):
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="Crypto Bot", callback_data=f"pay_crypto_{account_id}", style='primary', icon_custom_emoji_id=EMOJI_CRYPTO_BOT)
    )
    builder.row(
        InlineKeyboardButton(text="СБП", callback_data=f"pay_sbp_{account_id}", style='success', icon_custom_emoji_id=EMOJI_RECV_MONEY)
    )
    builder.row(
        InlineKeyboardButton(text="LZT Market", callback_data=f"pay_lzt_{account_id}", style='primary', icon_custom_emoji_id=EMOJI_MARKET)
    )
    builder.row(
        InlineKeyboardButton(text="Отмена", callback_data="cancel_payment", style='danger', icon_custom_emoji_id=EMOJI_CROSS)
    )
    return builder.as_markup()

def get_broadcast_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Отмена", callback_data="back_to_admin", style='danger', icon_custom_emoji_id=EMOJI_CROSS))
    return builder.as_markup()

def get_verify_payment_keyboard(purchase_id: int):
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="Одобрить", callback_data=f"approve_{purchase_id}", style='success', icon_custom_emoji_id=EMOJI_CHECK),
        InlineKeyboardButton(text="Отклонить", callback_data=f"reject_{purchase_id}", style='danger', icon_custom_emoji_id=EMOJI_CROSS)
    )
    return builder.as_markup()

def get_account_selection_keyboard(accounts):
    builder = InlineKeyboardBuilder()
    for acc in accounts:
        builder.row(InlineKeyboardButton(
            text=f"{acc[1]} - {acc[2]}₽",
            callback_data=f"select_acc_{acc[0]}",
            style='primary',
            icon_custom_emoji_id=EMOJI_WALLET
        ))
    builder.row(InlineKeyboardButton(text="Отмена", callback_data="cancel_selection", style='danger', icon_custom_emoji_id=EMOJI_CROSS))
    return builder.as_markup()

def get_edit_account_keyboard(acc_id: int):
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="Изменить цену", callback_data=f"edit_field_{acc_id}_price", style='primary', icon_custom_emoji_id=EMOJI_MONEY),
        InlineKeyboardButton(text="Изменить статус", callback_data=f"edit_field_{acc_id}_status", style='primary', icon_custom_emoji_id=EMOJI_TAG)
    )
    builder.row(
        InlineKeyboardButton(text="Удалить аккаунт", callback_data=f"delete_acc_{acc_id}", style='danger', icon_custom_emoji_id=EMOJI_TRASH),
        InlineKeyboardButton(text="Назад", callback_data="admin_manage_accs", style='default', icon_custom_emoji_id=EMOJI_BACK)
    )
    return builder.as_markup()

def get_manage_accounts_keyboard(accounts):
    builder = InlineKeyboardBuilder()
    for acc in accounts:
        builder.row(InlineKeyboardButton(
            text=f"#{acc[0]} {acc[1]} - {acc[2]}₽",
            callback_data=f"edit_acc_{acc[0]}",
            style='primary' if acc[4] == 'available' else 'default',
            icon_custom_emoji_id=EMOJI_PENCIL
        ))
    builder.row(InlineKeyboardButton(text="Назад", callback_data="back_to_admin", style='default', icon_custom_emoji_id=EMOJI_BACK))
    return builder.as_markup()

def get_search_results_keyboard(users):
    builder = InlineKeyboardBuilder()
    for user in users[:10]:
        builder.row(InlineKeyboardButton(
            text=f"ID: {user[0]} | {user[2]}",
            callback_data=f"user_details_{user[0]}",
            style='primary',
            icon_custom_emoji_id=EMOJI_PROFILE
        ))
    builder.row(InlineKeyboardButton(text="Назад", callback_data="back_to_admin", style='default', icon_custom_emoji_id=EMOJI_BACK))
    return builder.as_markup()

def get_requisites_keyboard():
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="Номер", callback_data="edit_req_sbp_phone", style='primary', icon_custom_emoji_id=EMOJI_PHONE),
        InlineKeyboardButton(text="Банк", callback_data="edit_req_sbp_bank", style='primary', icon_custom_emoji_id=EMOJI_BANK)
    )
    builder.row(
        InlineKeyboardButton(text="ФИО", callback_data="edit_req_sbp_fio", style='primary', icon_custom_emoji_id=EMOJI_PEOPLE),
        InlineKeyboardButton(text="Назад", callback_data="back_to_admin", style='default', icon_custom_emoji_id=EMOJI_BACK)
    )
    return builder.as_markup()

def get_crypto_payment_keyboard(invoice_url: str, purchase_id: int):
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Оплатить", url=invoice_url, style='success', icon_custom_emoji_id=EMOJI_SEND_MONEY))
    builder.row(InlineKeyboardButton(text="Проверить оплату", callback_data=f"check_crypto_{purchase_id}", style='primary', icon_custom_emoji_id=EMOJI_LOADING))
    return builder.as_markup()

def get_lzt_payment_keyboard(payment_url: str, purchase_id: int):
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Оплатить через LZT", url=payment_url, style='success', icon_custom_emoji_id=EMOJI_SEND_MONEY))
    builder.row(InlineKeyboardButton(text="Проверить оплату", callback_data=f"check_lzt_{purchase_id}", style='primary', icon_custom_emoji_id=EMOJI_LOADING))
    return builder.as_markup()

def get_code_keyboard(acc_id: int):
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Получить код", callback_data=f"get_code_{acc_id}", style='primary', icon_custom_emoji_id=EMOJI_CODE))
    return builder.as_markup()

# --- ОБРАБОТЧИКИ КОМАНД ---
@dp.message(Command("start"))
async def start_cmd(message: Message):
    user = message.from_user
    p = await get_pool()
    async with p.acquire() as conn:
        await conn.execute(
            "INSERT INTO users (user_id, username, first_name) VALUES ($1, $2, $3) ON CONFLICT (user_id) DO UPDATE SET username = $2, first_name = $3",
            user.id, user.username, user.first_name
        )
    
    if user.id in ADMIN_IDS:
        await message.answer(
            f'<b><tg-emoji emoji-id="{EMOJI_HOME}">🏘</tg-emoji> Добро пожаловать, Администратор!</b>',
            reply_markup=get_main_keyboard()
        )
        await message.answer(
            f'<b><tg-emoji emoji-id="{EMOJI_SETTINGS}">⚙</tg-emoji> Админ-панель:</b>',
            reply_markup=get_admin_keyboard()
        )
    else:
        await message.answer(
            f'<b><tg-emoji emoji-id="{EMOJI_HOME}">🏘</tg-emoji> Добро пожаловать в Vest Accounts!</b>\n\n'
            f'<tg-emoji emoji-id="{EMOJI_INFO}">ℹ</tg-emoji> Выберите действие в меню.',
            reply_markup=get_main_keyboard()
        )

@dp.message(Command("admin"))
async def admin_cmd(message: Message):
    if message.from_user.id not in ADMIN_IDS:
        return
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI_SETTINGS}">⚙</tg-emoji> Админ-панель:</b>',
        reply_markup=get_admin_keyboard()
    )

@dp.message(F.text == "Профиль")
async def profile(message: Message):
    user_id = message.from_user.id
    p = await get_pool()
    async with p.acquire() as conn:
        user_data = await conn.fetchrow("SELECT username, first_name, created_at FROM users WHERE user_id = $1", user_id)
        
        if not user_data:
            await message.answer("Пользователь не найден")
            return
        
        username, first_name, created_at = user_data
        
        purchases = await conn.fetch("""
            SELECT p.id, a.country, a.media_file_id, a.media_type, p.amount, p.created_at, p.status 
            FROM purchases p
            JOIN accounts a ON p.account_id = a.id
            WHERE p.user_id = $1 AND p.status = 'completed'
            ORDER BY p.created_at DESC LIMIT 10
        """, user_id)
        
        total_purchases = await conn.fetchval("SELECT COUNT(*) FROM purchases WHERE user_id = $1 AND status = 'completed'", user_id)
        total_spent = await conn.fetchval("SELECT SUM(amount) FROM purchases WHERE user_id = $1 AND status = 'completed'", user_id) or 0
    
    text = f'<b><tg-emoji emoji-id="{EMOJI_PROFILE}">👤</tg-emoji> Ваш профиль</b>\n\n'
    text += f'<tg-emoji emoji-id="{EMOJI_TAG}">🏷</tg-emoji> ID: <code>{user_id}</code>\n'
    text += f'<tg-emoji emoji-id="{EMOJI_PEOPLE}">👥</tg-emoji> Имя: {first_name}\n'
    text += f'<tg-emoji emoji-id="{EMOJI_LINK}">🔗</tg-emoji> Username: @{username if username else "не указан"}\n'
    text += f'<tg-emoji emoji-id="{EMOJI_CALENDAR}">📅</tg-emoji> Дата регистрации: {str(created_at)[:10]}\n\n'
    
    text += f'<b><tg-emoji emoji-id="{EMOJI_STATS}">📊</tg-emoji> Статистика покупок:</b>\n'
    text += f'<tg-emoji emoji-id="{EMOJI_BOX}">📦</tg-emoji> Всего покупок: {total_purchases}\n'
    text += f'<tg-emoji emoji-id="{EMOJI_MONEY}">🪙</tg-emoji> Потрачено: {total_spent}₽\n\n'
    
    await message.answer(text)
    
    if purchases:
        for p in purchases[:5]:
            if p[2] and p[3]:
                caption = f'<tg-emoji emoji-id="{EMOJI_LOCK_OPEN}">🔓</tg-emoji> {p[1]} - {p[4]}₽ ({str(p[5])[:10]})'
                try:
                    if p[3] == "photo":
                        await message.answer_photo(p[2], caption=caption)
                    elif p[3] == "video":
                        await message.answer_video(p[2], caption=caption)
                    elif p[3] == "animation":
                        await message.answer_animation(p[2], caption=caption)
                except:
                    await message.answer(caption)
            else:
                await message.answer(f'<tg-emoji emoji-id="{EMOJI_LOCK_OPEN}">🔓</tg-emoji> {p[1]} - {p[4]}₽ ({str(p[5])[:10]})')

@dp.message(F.text == "Поддержка")
async def support(message: Message):
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Написать в поддержку", url="https://t.me/VestSupport", style='primary', icon_custom_emoji_id=EMOJI_SEND))
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI_INFO}">ℹ</tg-emoji> Поддержка Vest Accounts</b>\n\n'
        f'<tg-emoji emoji-id="{EMOJI_SEND}">⬆</tg-emoji> По всем вопросам обращайтесь: @VestSupport',
        reply_markup=builder.as_markup()
    )

@dp.message(F.text == "Купить аккаунт")
async def buy_account(message: Message):
    p = await get_pool()
    async with p.acquire() as conn:
        accounts = await conn.fetch(
            "SELECT id, country, price, media_file_id, media_type, description FROM accounts WHERE status = 'available' ORDER BY price ASC"
        )
    
    if not accounts:
        await message.answer(
            f'<b><tg-emoji emoji-id="{EMOJI_CROSS}">❌</tg-emoji> Нет доступных аккаунтов</b>\n\n'
            f'<tg-emoji emoji-id="{EMOJI_CLOCK}">⏰</tg-emoji> Загляните позже!'
        )
        return
    
    text = f'<b><tg-emoji emoji-id="{EMOJI_BOX}">📦</tg-emoji> Доступные аккаунты ({len(accounts)}):</b>\n\n'
    
    for acc in accounts:
        desc = f" - {acc[5]}" if acc[5] else ""
        text += f'<tg-emoji emoji-id="{EMOJI_GEO}">📍</tg-emoji> {acc[1]} | <tg-emoji emoji-id="{EMOJI_MONEY}">🪙</tg-emoji> {acc[2]}₽{desc}\n'
    
    await message.answer(text, reply_markup=get_account_selection_keyboard(accounts))
    
    for acc in accounts:
        if acc[3] and acc[4]:
            caption = f'<tg-emoji emoji-id="{EMOJI_GEO}">📍</tg-emoji> {acc[1]} | <tg-emoji emoji-id="{EMOJI_MONEY}">🪙</tg-emoji> {acc[2]}₽'
            try:
                if acc[4] == "photo":
                    await message.answer_photo(acc[3], caption=caption)
                elif acc[4] == "video":
                    await message.answer_video(acc[3], caption=caption)
                elif acc[4] == "animation":
                    await message.answer_animation(acc[3], caption=caption)
            except:
                pass

@dp.callback_query(F.data.startswith("select_acc_"))
async def select_account(callback: CallbackQuery):
    acc_id = int(callback.data.split("_")[2])
    
    p = await get_pool()
    async with p.acquire() as conn:
        acc = await conn.fetchrow(
            "SELECT country, price, media_file_id, media_type, description FROM accounts WHERE id = $1 AND status = 'available'",
            acc_id
        )
    
    if not acc:
        await callback.answer("Аккаунт недоступен", show_alert=True)
        return
    
    text = f'<b><tg-emoji emoji-id="{EMOJI_GEO}">📍</tg-emoji> Выбран аккаунт:</b>\n'
    text += f'<tg-emoji emoji-id="{EMOJI_TAG}">🏷</tg-emoji> Страна: {acc[0]}\n'
    text += f'<tg-emoji emoji-id="{EMOJI_MONEY}">🪙</tg-emoji> Цена: {acc[1]}₽\n'
    if acc[4]:
        text += f'<tg-emoji emoji-id="{EMOJI_INFO}">ℹ</tg-emoji> Описание: {acc[4]}\n'
    text += f'\n<b><tg-emoji emoji-id="{EMOJI_WALLET}">👛</tg-emoji> Выберите способ оплаты:</b>'
    
    if acc[2] and acc[3]:
        try:
            if acc[3] == "photo":
                await callback.message.answer_photo(acc[2], caption=text, reply_markup=get_payment_method_keyboard(acc_id))
            elif acc[3] == "video":
                await callback.message.answer_video(acc[2], caption=text, reply_markup=get_payment_method_keyboard(acc_id))
            elif acc[3] == "animation":
                await callback.message.answer_animation(acc[2], caption=text, reply_markup=get_payment_method_keyboard(acc_id))
            await callback.message.delete()
        except:
            await callback.message.edit_text(text, reply_markup=get_payment_method_keyboard(acc_id))
    else:
        await callback.message.edit_text(text, reply_markup=get_payment_method_keyboard(acc_id))
    
    await callback.answer()

# --- ОПЛАТА CRYPTO BOT ---
@dp.callback_query(F.data.startswith("pay_crypto_"))
async def pay_crypto(callback: CallbackQuery):
    acc_id = int(callback.data.split("_")[2])
    user_id = callback.from_user.id
    
    p = await get_pool()
    async with p.acquire() as conn:
        acc = await conn.fetchrow("SELECT price FROM accounts WHERE id = $1", acc_id)
        if not acc:
            await callback.answer("Ошибка: аккаунт не найден", show_alert=True)
            return
        price_rub = acc[0]
        
        purchase_id = await conn.fetchval("""
            INSERT INTO purchases (user_id, account_id, payment_method, amount, status)
            VALUES ($1, $2, 'crypto', $3, 'pending')
            RETURNING id
        """, user_id, acc_id, price_rub)
    
    usdt_amount = round(price_rub / 90, 2)
    
    try:
        async with aiohttp.ClientSession() as session:
            headers = {"Crypto-Pay-API-Token": CRYPTO_BOT_TOKEN}
            json_data = {
                "asset": "USDT",
                "amount": str(usdt_amount),
                "description": f"Покупка аккаунта #{acc_id}",
                "payload": f"purchase_{purchase_id}",
                "allow_comments": False,
                "allow_anonymous": False
            }
            async with session.post("https://pay.crypt.bot/api/createInvoice", headers=headers, json=json_data) as resp:
                data = await resp.json()
                logging.info(f"Crypto Bot response: {data}")
        
        if data.get("ok"):
            invoice_url = data["result"]["pay_url"]
            await callback.message.edit_text(
                f'<b><tg-emoji emoji-id="{EMOJI_CRYPTO_BOT}">👾</tg-emoji> Счёт Crypto Bot создан!</b>\n\n'
                f'<tg-emoji emoji-id="{EMOJI_MONEY}">🪙</tg-emoji> Сумма: {usdt_amount} USDT (~{price_rub}₽)\n'
                f'<tg-emoji emoji-id="{EMOJI_LINK}">🔗</tg-emoji> <a href="{invoice_url}">Нажмите для оплаты</a>\n\n'
                f'<tg-emoji emoji-id="{EMOJI_CLOCK}">⏰</tg-emoji> После оплаты нажмите кнопку ниже.',
                reply_markup=get_crypto_payment_keyboard(invoice_url, purchase_id),
                disable_web_page_preview=True
            )
        else:
            error_msg = data.get("error", "Неизвестная ошибка")
            await callback.message.edit_text(
                f'<b><tg-emoji emoji-id="{EMOJI_CROSS}">❌</tg-emoji> Ошибка создания счёта</b>\n\n{error_msg}',
                reply_markup=get_back_button()
            )
    except Exception as e:
        logging.error(f"Crypto Bot error: {e}")
        await callback.message.edit_text(
            f'<b><tg-emoji emoji-id="{EMOJI_CROSS}">❌</tg-emoji> Ошибка подключения к Crypto Bot</b>',
            reply_markup=get_back_button()
        )
    await callback.answer()

@dp.callback_query(F.data.startswith("check_crypto_"))
async def check_crypto_payment(callback: CallbackQuery):
    purchase_id = int(callback.data.split("_")[2])
    
    try:
        async with aiohttp.ClientSession() as session:
            headers = {"Crypto-Pay-API-Token": CRYPTO_BOT_TOKEN}
            async with session.get("https://pay.crypt.bot/api/getInvoices", headers=headers) as resp:
                data = await resp.json()
                logging.info(f"Crypto Bot check response: {data}")
        
        paid = False
        if data.get("ok"):
            for inv in data["result"]["items"]:
                if inv["payload"] == f"purchase_{purchase_id}" and inv["status"] == "paid":
                    paid = True
                    break
        
        if paid:
            p = await get_pool()
            async with p.acquire() as conn:
                purchase = await conn.fetchrow("SELECT account_id, user_id FROM purchases WHERE id = $1", purchase_id)
                if purchase:
                    acc_id, user_id = purchase
                    await complete_purchase(purchase_id, acc_id, user_id, callback)
            await callback.answer("Оплата подтверждена!", show_alert=True)
        else:
            await callback.answer("Оплата ещё не поступила. Попробуйте позже.", show_alert=True)
    except Exception as e:
        logging.error(f"Crypto Bot check error: {e}")
        await callback.answer("Ошибка проверки оплаты", show_alert=True)

# --- ОПЛАТА LZT.MARKET ---
@dp.callback_query(F.data.startswith("pay_lzt_"))
async def pay_lzt(callback: CallbackQuery):
    acc_id = int(callback.data.split("_")[2])
    user_id = callback.from_user.id
    
    p = await get_pool()
    async with p.acquire() as conn:
        acc = await conn.fetchrow("SELECT price FROM accounts WHERE id = $1", acc_id)
        if not acc:
            await callback.answer("Ошибка: аккаунт не найден", show_alert=True)
            return
        price_rub = acc[0]
        
        purchase_id = await conn.fetchval("""
            INSERT INTO purchases (user_id, account_id, payment_method, amount, status)
            VALUES ($1, $2, 'lzt', $3, 'pending')
            RETURNING id
        """, user_id, acc_id, price_rub)
    
    payment_id = f"purchase_{purchase_id}_{int(datetime.now().timestamp())}"
    
    try:
        result = await create_lzt_invoice(
            amount=price_rub,
            payment_id=payment_id,
            comment=f"Покупка аккаунта #{acc_id}",
            user_id=user_id
        )
        logging.info(f"LZT response: {result}")
        
        if "invoice" in result and "url" in result["invoice"]:
            invoice_url = result["invoice"]["url"]
            lzt_invoice_id = result["invoice"]["invoice_id"]
            
            async with p.acquire() as conn:
                await conn.execute(
                    "UPDATE purchases SET lzt_invoice_id = $1, lzt_payment_id = $2 WHERE id = $3",
                    str(lzt_invoice_id), payment_id, purchase_id
                )
            
            await callback.message.edit_text(
                f'<b><tg-emoji emoji-id="{EMOJI_MARKET}">🎁</tg-emoji> Счёт LZT.Market создан!</b>\n\n'
                f'<tg-emoji emoji-id="{EMOJI_MONEY}">🪙</tg-emoji> Сумма: {price_rub}₽\n'
                f'<tg-emoji emoji-id="{EMOJI_LINK}">🔗</tg-emoji> <a href="{invoice_url}">Нажмите для оплаты</a>\n\n'
                f'<tg-emoji emoji-id="{EMOJI_CLOCK}">⏰</tg-emoji> После оплаты нажмите кнопку ниже.',
                reply_markup=get_lzt_payment_keyboard(invoice_url, purchase_id),
                disable_web_page_preview=True
            )
        else:
            await conn.execute("UPDATE purchases SET status = 'failed' WHERE id = $1", purchase_id)
            error_text = "Неизвестная ошибка"
            if "errors" in result:
                error_text = result["errors"][0] if isinstance(result["errors"], list) else str(result["errors"])
            await callback.message.edit_text(
                f'<b><tg-emoji emoji-id="{EMOJI_CROSS}">❌</tg-emoji> Ошибка создания счёта LZT</b>\n\n{error_text}',
                reply_markup=get_back_button()
            )
    except Exception as e:
        logging.error(f"LZT error: {e}")
        await conn.execute("UPDATE purchases SET status = 'failed' WHERE id = $1", purchase_id)
        await callback.message.edit_text(
            f'<b><tg-emoji emoji-id="{EMOJI_CROSS}">❌</tg-emoji> Ошибка подключения к LZT Market</b>',
            reply_markup=get_back_button()
        )
    await callback.answer()

@dp.callback_query(F.data.startswith("check_lzt_"))
async def check_lzt_payment(callback: CallbackQuery):
    purchase_id = int(callback.data.split("_")[2])
    
    p = await get_pool()
    async with p.acquire() as conn:
        purchase = await conn.fetchrow("SELECT lzt_invoice_id, account_id, user_id FROM purchases WHERE id = $1", purchase_id)
        
        if not purchase or not purchase[0]:
            await callback.answer("Счёт не найден", show_alert=True)
            return
        
        lzt_invoice_id, acc_id, user_id = purchase
        
        try:
            result = await check_lzt_invoice(int(lzt_invoice_id))
            logging.info(f"LZT check response: {result}")
            
            if "invoice" in result:
                status = result["invoice"].get("status", "")
                if status == "paid":
                    await complete_purchase(purchase_id, acc_id, user_id, callback)
                    await callback.answer("Оплата подтверждена!", show_alert=True)
                    return
                elif status == "expired":
                    await conn.execute("UPDATE purchases SET status = 'expired' WHERE id = $1", purchase_id)
                    await callback.message.edit_text(
                        f'<b><tg-emoji emoji-id="{EMOJI_CROSS}">❌</tg-emoji> Время оплаты истекло</b>',
                        reply_markup=get_back_button()
                    )
                    await callback.answer("Счёт просрочен", show_alert=True)
                    return
            
            await callback.answer("Оплата ещё не поступила. Попробуйте позже.", show_alert=True)
        except Exception as e:
            logging.error(f"LZT check error: {e}")
            await callback.answer("Ошибка проверки оплаты", show_alert=True)

# --- ОПЛАТА СБП ---
@dp.callback_query(F.data.startswith("pay_sbp_"))
async def pay_sbp(callback: CallbackQuery, state: FSMContext):
    acc_id = int(callback.data.split("_")[2])
    user_id = callback.from_user.id
    
    p = await get_pool()
    async with p.acquire() as conn:
        acc = await conn.fetchrow("SELECT price FROM accounts WHERE id = $1", acc_id)
        if not acc:
            await callback.answer("Ошибка", show_alert=True)
            return
        price = acc[0]
        
        reqs = {}
        rows = await conn.fetch("SELECT key, value FROM requisites WHERE key IN ('sbp_phone', 'sbp_bank', 'sbp_fio')")
        for row in rows:
            reqs[row['key']] = row['value']
        
        purchase_id = await conn.fetchval("""
            INSERT INTO purchases (user_id, account_id, payment_method, amount, status)
            VALUES ($1, $2, 'sbp', $3, 'pending')
            RETURNING id
        """, user_id, acc_id, price)
    
    await state.update_data(purchase_id=purchase_id, acc_id=acc_id)
    await state.set_state(PaymentSBP.waiting_for_screenshot)
    
    text = f'<b><tg-emoji emoji-id="{EMOJI_RECV_MONEY}">🏧</tg-emoji> Оплата через СБП</b>\n\n'
    text += f'<tg-emoji emoji-id="{EMOJI_MONEY}">🪙</tg-emoji> Сумма: {price}₽\n\n'
    text += f'<b><tg-emoji emoji-id="{EMOJI_INFO}">ℹ</tg-emoji> Реквизиты для оплаты:</b>\n'
    text += f'<tg-emoji emoji-id="{EMOJI_PHONE}">📱</tg-emoji> Номер: <code>{reqs.get("sbp_phone", "не указан")}</code>\n'
    text += f'<tg-emoji emoji-id="{EMOJI_BANK}">🏦</tg-emoji> Банк: {reqs.get("sbp_bank", "не указан")}\n'
    text += f'<tg-emoji emoji-id="{EMOJI_PEOPLE}">👥</tg-emoji> ФИО: {reqs.get("sbp_fio", "не указано")}\n\n'
    text += f'<b><tg-emoji emoji-id="{EMOJI_MEDIA}">🖼</tg-emoji> Пришлите скриншот успешной оплаты:</b>'
    
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Отмена", callback_data="cancel_payment", style='danger', icon_custom_emoji_id=EMOJI_CROSS))
    
    await callback.message.edit_text(text, reply_markup=builder.as_markup())
    await callback.answer()

@dp.message(StateFilter(PaymentSBP.waiting_for_screenshot), F.photo)
async def receive_screenshot(message: Message, state: FSMContext):
    data = await state.get_data()
    purchase_id = data['purchase_id']
    user_id = message.from_user.id
    
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_photo(
                admin_id,
                message.photo[-1].file_id,
                caption=f'<b><tg-emoji emoji-id="{EMOJI_MEDIA}">🖼</tg-emoji> Новый скриншот оплаты СБП</b>\n\n'
                        f'<tg-emoji emoji-id="{EMOJI_TAG}">🏷</tg-emoji> Покупка #{purchase_id}\n'
                        f'<tg-emoji emoji-id="{EMOJI_PROFILE}">👤</tg-emoji> Пользователь: @{message.from_user.username} ({user_id})',
                reply_markup=get_verify_payment_keyboard(purchase_id)
            )
        except Exception as e:
            logging.error(f"Failed to send to admin {admin_id}: {e}")
    
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI_CHECK}">✅</tg-emoji> Скриншот отправлен на проверку!</b>\n'
        f'<tg-emoji emoji-id="{EMOJI_CLOCK}">⏰</tg-emoji> Ожидайте подтверждения от администратора.',
        reply_markup=get_main_keyboard()
    )
    await state.clear()

@dp.callback_query(F.data.startswith("approve_"))
async def approve_payment(callback: CallbackQuery):
    purchase_id = int(callback.data.split("_")[1])
    
    p = await get_pool()
    async with p.acquire() as conn:
        purchase = await conn.fetchrow("SELECT account_id, user_id FROM purchases WHERE id = $1", purchase_id)
        if purchase:
            acc_id, user_id = purchase
            await complete_purchase(purchase_id, acc_id, user_id)
            
            await callback.message.edit_caption(
                caption=f"{callback.message.caption}\n\n<b><tg-emoji emoji-id=\"{EMOJI_CHECK}\">✅</tg-emoji> ОДОБРЕНО</b>"
            )
    
    await callback.answer("Одобрено!")

@dp.callback_query(F.data.startswith("reject_"))
async def reject_payment(callback: CallbackQuery):
    purchase_id = int(callback.data.split("_")[1])
    
    p = await get_pool()
    async with p.acquire() as conn:
        purchase = await conn.fetchrow("SELECT user_id FROM purchases WHERE id = $1", purchase_id)
        if purchase:
            user_id = purchase[0]
            await conn.execute("UPDATE purchases SET status = 'rejected' WHERE id = $1", purchase_id)
            
            try:
                await bot.send_message(
                    user_id,
                    f'<b><tg-emoji emoji-id="{EMOJI_CROSS}">❌</tg-emoji> Оплата отклонена</b>\n\n'
                    f'<tg-emoji emoji-id="{EMOJI_INFO}">ℹ</tg-emoji> Свяжитесь с поддержкой для уточнения деталей.'
                )
            except:
                pass
    
    await callback.message.edit_caption(
        caption=f"{callback.message.caption}\n\n<b><tg-emoji emoji-id=\"{EMOJI_CROSS}\">❌</tg-emoji> ОТКЛОНЕНО</b>"
    )
    await callback.answer("Отклонено")

@dp.callback_query(F.data.startswith("get_code_"))
async def get_code(callback: CallbackQuery):
    acc_id = int(callback.data.split("_")[2])
    
    await callback.answer()
    
    p = await get_pool()
    async with p.acquire() as conn:
        acc = await conn.fetchrow("SELECT phone, twofa FROM accounts WHERE id = $1", acc_id)
    
    if not acc:
        await callback.answer("Аккаунт не найден", show_alert=True)
        return
    
    phone, twofa = acc
    
    # Получаем код через Telegram
    code = await login_and_get_code(phone, twofa)
    
    text = f'<b><tg-emoji emoji-id="{EMOJI_CODE}">🔨</tg-emoji> Данные аккаунта:</b>\n\n'
    text += f'<tg-emoji emoji-id="{EMOJI_PHONE}">📱</tg-emoji> Номер: <code>{phone}</code>\n'
    
    if code and code != "already_authorized":
        text += f'<tg-emoji emoji-id="{EMOJI_KEY}">🔑</tg-emoji> Код: <code>{code}</code>\n'
    elif code == "already_authorized":
        text += f'<tg-emoji emoji-id="{EMOJI_INFO}">ℹ</tg-emoji> Аккаунт уже авторизован\n'
    else:
        text += f'<tg-emoji emoji-id="{EMOJI_CROSS}">❌</tg-emoji> Не удалось получить код\n'
        text += f'<tg-emoji emoji-id="{EMOJI_INFO}">ℹ</tg-emoji> Попробуйте позже или обратитесь в поддержку\n'
    
    if twofa:
        text += f'<tg-emoji emoji-id="{EMOJI_LOCK_CLOSED}">🔒</tg-emoji> 2FA пароль: <code>{twofa}</code>\n'
    
    await callback.message.edit_text(text)
    await callback.answer()

# --- АДМИН ПАНЕЛЬ ---
@dp.callback_query(F.data == "admin_stats")
async def admin_stats(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Нет доступа", show_alert=True)
        return
    
    p = await get_pool()
    async with p.acquire() as conn:
        users_count = await conn.fetchval("SELECT COUNT(*) FROM users")
        available = await conn.fetchval("SELECT COUNT(*) FROM accounts WHERE status = 'available'")
        sold = await conn.fetchval("SELECT COUNT(*) FROM accounts WHERE status = 'sold'")
        completed_purchases = await conn.fetchval("SELECT COUNT(*) FROM purchases WHERE status = 'completed'")
        pending_purchases = await conn.fetchval("SELECT COUNT(*) FROM purchases WHERE status = 'pending'")
        revenue = await conn.fetchval("SELECT SUM(amount) FROM purchases WHERE status = 'completed'") or 0
        avg_check = await conn.fetchval("SELECT AVG(amount) FROM purchases WHERE status = 'completed'") or 0
        payment_stats = await conn.fetch("""
            SELECT payment_method, COUNT(*) FROM purchases 
            WHERE status = 'completed' GROUP BY payment_method
        """)
        repeat_buyers = await conn.fetchval("SELECT COUNT(*) FROM (SELECT user_id FROM purchases WHERE status = 'completed' GROUP BY user_id HAVING COUNT(*) > 1) as t")
    
    text = f'<b><tg-emoji emoji-id="{EMOJI_STATS}">📊</tg-emoji> Статистика</b>\n\n'
    text += f'<b><tg-emoji emoji-id="{EMOJI_PEOPLE}">👥</tg-emoji> Пользователи:</b>\n'
    text += f'├ Всего: {users_count}\n'
    text += f'└ Повторных покупателей: {repeat_buyers}\n\n'
    
    text += f'<b><tg-emoji emoji-id="{EMOJI_BOX}">📦</tg-emoji> Аккаунты:</b>\n'
    text += f'├ Доступно: {available}\n'
    text += f'└ Продано: {sold}\n\n'
    
    text += f'<b><tg-emoji emoji-id="{EMOJI_WALLET}">👛</tg-emoji> Продажи:</b>\n'
    text += f'├ Всего: {completed_purchases}\n'
    text += f'├ В обработке: {pending_purchases}\n'
    text += f'├ Выручка: {revenue}₽\n'
    text += f'└ Средний чек: {avg_check:.0f}₽\n\n'
    
    if payment_stats:
        method_names = {"crypto": "Crypto Bot", "sbp": "СБП", "lzt": "LZT Market"}
        text += f'<b><tg-emoji emoji-id="{EMOJI_RECV_MONEY}">🏧</tg-emoji> По способам оплаты:</b>\n'
        for row in payment_stats:
            text += f'├ {method_names.get(row[0], row[0])}: {row[1]}\n'
    
    await callback.message.edit_text(text, reply_markup=get_back_button())
    await callback.answer()

@dp.callback_query(F.data == "admin_manage_accs")
async def admin_manage_accs(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Нет доступа", show_alert=True)
        return
    
    p = await get_pool()
    async with p.acquire() as conn:
        accounts = await conn.fetch(
            "SELECT id, country, price, phone, status FROM accounts ORDER BY created_at DESC LIMIT 15"
        )
    
    if not accounts:
        await callback.message.edit_text(
            f'<b><tg-emoji emoji-id="{EMOJI_INFO}">ℹ</tg-emoji> Нет аккаунтов в базе</b>',
            reply_markup=get_back_button()
        )
        return
    
    text = f'<b><tg-emoji emoji-id="{EMOJI_LIST}">📋</tg-emoji> Управление аккаунтами:</b>\n\n'
    
    for acc in accounts:
        status_emoji = EMOJI_CHECK if acc[4] == 'available' else EMOJI_LOCK_CLOSED
        status_text = "Доступен" if acc[4] == 'available' else "Продан"
        text += f'<tg-emoji emoji-id="{status_emoji}"></tg-emoji> #{acc[0]} | {acc[1]} | {acc[2]}₽ | {acc[3]} | {status_text}\n'
    
    await callback.message.edit_text(text, reply_markup=get_manage_accounts_keyboard(accounts))
    await callback.answer()

@dp.callback_query(F.data.startswith("edit_acc_"))
async def edit_acc_menu(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Нет доступа", show_alert=True)
        return
    
    acc_id = int(callback.data.split("_")[2])
    
    p = await get_pool()
    async with p.acquire() as conn:
        acc = await conn.fetchrow(
            "SELECT country, price, phone, twofa, status FROM accounts WHERE id = $1", acc_id
        )
    
    if not acc:
        await callback.answer("Аккаунт не найден", show_alert=True)
        return
    
    text = f'<b><tg-emoji emoji-id="{EMOJI_PENCIL}">🖋</tg-emoji> Редактирование аккаунта #{acc_id}</b>\n\n'
    text += f'<tg-emoji emoji-id="{EMOJI_GEO}">📍</tg-emoji> Страна: {acc[0]}\n'
    text += f'<tg-emoji emoji-id="{EMOJI_MONEY}">🪙</tg-emoji> Цена: {acc[1]}₽\n'
    text += f'<tg-emoji emoji-id="{EMOJI_PHONE}">📱</tg-emoji> Номер: {acc[2]}\n'
    text += f'<tg-emoji emoji-id="{EMOJI_LOCK_CLOSED}">🔒</tg-emoji> 2FA: {acc[3] or "нет"}\n'
    text += f'<tg-emoji emoji-id="{EMOJI_TAG}">🏷</tg-emoji> Статус: {"Доступен" if acc[4] == "available" else "Продан"}\n\n'
    text += f'<b>Выберите действие:</b>'
    
    await callback.message.edit_text(text, reply_markup=get_edit_account_keyboard(acc_id))
    await callback.answer()

@dp.callback_query(F.data.startswith("edit_field_"))
async def edit_field_start(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Нет доступа", show_alert=True)
        return
    
    parts = callback.data.split("_")
    acc_id = int(parts[2])
    field = parts[3]
    
    await state.update_data(edit_acc_id=acc_id, edit_field=field)
    await state.set_state(EditAccount.waiting_for_value)
    
    field_names = {"price": "цену", "status": "статус (available/sold)"}
    
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Отмена", callback_data=f"edit_acc_{acc_id}", style='danger', icon_custom_emoji_id=EMOJI_CROSS))
    
    await callback.message.edit_text(
        f'<b><tg-emoji emoji-id="{EMOJI_PENCIL}">🖋</tg-emoji> Введите новое значение для "{field_names[field]}":</b>',
        reply_markup=builder.as_markup()
    )
    await callback.answer()

@dp.message(StateFilter(EditAccount.waiting_for_value))
async def edit_field_save(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    
    data = await state.get_data()
    acc_id = data['edit_acc_id']
    field = data['edit_field']
    value = message.text.strip()
    
    if field == "price":
        try:
            value = int(value)
        except ValueError:
            await message.answer("Введите число!")
            return
    elif field == "status":
        if value not in ["available", "sold"]:
            await message.answer("Введите available или sold!")
            return
    
    p = await get_pool()
    async with p.acquire() as conn:
        await conn.execute(f"UPDATE accounts SET {field} = $1 WHERE id = $2", value, acc_id)
    
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI_CHECK}">✅</tg-emoji> Значение обновлено!</b>',
        reply_markup=get_main_keyboard()
    )
    await state.clear()

@dp.callback_query(F.data.startswith("delete_acc_"))
async def delete_account(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Нет доступа", show_alert=True)
        return
    
    acc_id = int(callback.data.split("_")[2])
    
    p = await get_pool()
    async with p.acquire() as conn:
        await conn.execute("DELETE FROM accounts WHERE id = $1", acc_id)
    
    await callback.answer("Аккаунт удалён", show_alert=True)
    await admin_manage_accs(callback)

@dp.callback_query(F.data == "admin_search_user")
async def admin_search_user(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Нет доступа", show_alert=True)
        return
    
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Отмена", callback_data="back_to_admin", style='danger', icon_custom_emoji_id=EMOJI_CROSS))
    
    await callback.message.edit_text(
        f'<b><tg-emoji emoji-id="{EMOJI_SEARCH}">🔍</tg-emoji> Поиск пользователя</b>\n\n'
        f'Введите ID, username или имя пользователя:',
        reply_markup=builder.as_markup()
    )
    await state.set_state(SearchUser.waiting_for_query)
    await callback.answer()

@dp.message(StateFilter(SearchUser.waiting_for_query))
async def search_user_result(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    
    query = message.text.strip()
    
    p = await get_pool()
    async with p.acquire() as conn:
        if query.isdigit():
            users = await conn.fetch(
                "SELECT user_id, username, first_name, created_at FROM users WHERE user_id = $1",
                int(query)
            )
        elif query.startswith("@"):
            users = await conn.fetch(
                "SELECT user_id, username, first_name, created_at FROM users WHERE username = $1",
                query[1:]
            )
        else:
            users = await conn.fetch(
                "SELECT user_id, username, first_name, created_at FROM users WHERE first_name ILIKE $1",
                f"%{query}%"
            )
    
    if not users:
        await message.answer(
            f'<b><tg-emoji emoji-id="{EMOJI_CROSS}">❌</tg-emoji> Пользователь не найден</b>',
            reply_markup=get_back_button()
        )
        await state.clear()
        return
    
    text = f'<b><tg-emoji emoji-id="{EMOJI_SEARCH}">🔍</tg-emoji> Результаты поиска ({len(users)}):</b>\n\n'
    
    for user in users[:10]:
        text += f'<tg-emoji emoji-id="{EMOJI_PROFILE}">👤</tg-emoji> ID: <code>{user[0]}</code>\n'
        text += f'├ Имя: {user[2]}\n'
        text += f'├ Username: @{user[1] or "нет"}\n'
        text += f'└ Регистрация: {str(user[3])[:10]}\n\n'
    
    await message.answer(text, reply_markup=get_search_results_keyboard(users))
    await state.clear()

@dp.callback_query(F.data.startswith("user_details_"))
async def user_details(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Нет доступа", show_alert=True)
        return
    
    user_id = int(callback.data.split("_")[2])
    
    p = await get_pool()
    async with p.acquire() as conn:
        user = await conn.fetchrow(
            "SELECT username, first_name, created_at FROM users WHERE user_id = $1", user_id
        )
        
        purchases = await conn.fetch("""
            SELECT p.id, a.country, p.amount, p.created_at, p.status, p.payment_method
            FROM purchases p
            JOIN accounts a ON p.account_id = a.id
            WHERE p.user_id = $1
            ORDER BY p.created_at DESC
        """, user_id)
        
        total_spent = await conn.fetchval("SELECT SUM(amount) FROM purchases WHERE user_id = $1 AND status = 'completed'", user_id) or 0
    
    if not user:
        await callback.answer("Пользователь не найден", show_alert=True)
        return
    
    username, first_name, created_at = user
    
    text = f'<b><tg-emoji emoji-id="{EMOJI_PROFILE}">👤</tg-emoji> Информация о пользователе</b>\n\n'
    text += f'<tg-emoji emoji-id="{EMOJI_TAG}">🏷</tg-emoji> ID: <code>{user_id}</code>\n'
    text += f'<tg-emoji emoji-id="{EMOJI_PEOPLE}">👥</tg-emoji> Имя: {first_name}\n'
    text += f'<tg-emoji emoji-id="{EMOJI_LINK}">🔗</tg-emoji> Username: @{username or "нет"}\n'
    text += f'<tg-emoji emoji-id="{EMOJI_CALENDAR}">📅</tg-emoji> Регистрация: {str(created_at)[:10]}\n'
    text += f'<tg-emoji emoji-id="{EMOJI_MONEY}">🪙</tg-emoji> Всего потрачено: {total_spent}₽\n\n'
    
    if purchases:
        method_names = {"crypto": "Crypto Bot", "sbp": "СБП", "lzt": "LZT Market"}
        text += f'<b><tg-emoji emoji-id="{EMOJI_BOX}">📦</tg-emoji> Покупки ({len(purchases)}):</b>\n'
        for p in purchases[:10]:
            status_emoji = EMOJI_CHECK if p[4] == 'completed' else EMOJI_CLOCK if p[4] == 'pending' else EMOJI_CROSS
            text += f'<tg-emoji emoji-id="{status_emoji}"></tg-emoji> #{p[0]} | {p[1]} | {p[2]}₽ | {method_names.get(p[5], p[5])} | {str(p[3])[:10]}\n'
    
    await callback.message.edit_text(text, reply_markup=get_back_button("admin_search_user"))
    await callback.answer()

@dp.callback_query(F.data == "admin_export_db")
async def admin_export_db(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Нет доступа", show_alert=True)
        return
    
    import csv
    import io
    
    p = await get_pool()
    async with p.acquire() as conn:
        users = await conn.fetch("SELECT * FROM users")
        accounts = await conn.fetch("SELECT id, country, price, phone, twofa, status, created_at FROM accounts")
        purchases = await conn.fetch("SELECT * FROM purchases")
    
    output = io.StringIO()
    writer = csv.writer(output)
    
    writer.writerow(["=== ПОЛЬЗОВАТЕЛИ ==="])
    writer.writerow(["user_id", "username", "first_name", "created_at"])
    for u in users:
        writer.writerow([u[0], u[1], u[2], str(u[3])])
    
    writer.writerow([])
    writer.writerow(["=== АККАУНТЫ ==="])
    writer.writerow(["id", "country", "price", "phone", "twofa", "status", "created_at"])
    for a in accounts:
        writer.writerow([a[0], a[1], a[2], a[3], a[4], a[5], str(a[6])])
    
    writer.writerow([])
    writer.writerow(["=== ПОКУПКИ ==="])
    writer.writerow(["id", "user_id", "account_id", "payment_method", "amount", "status", "lzt_invoice_id", "lzt_payment_id", "created_at"])
    for p in purchases:
        row = [p[0], p[1], p[2], p[3], p[4], p[5]]
        row.append(p[6] if len(p) > 6 else "")
        row.append(p[7] if len(p) > 7 else "")
        row.append(str(p[8] if len(p) > 8 else p[6]))
        writer.writerow(row)
    
    output.seek(0)
    
    from aiogram.types import BufferedInputFile
    file = BufferedInputFile(output.getvalue().encode('utf-8'), filename="vest_accounts_export.csv")
    
    await callback.message.answer_document(
        file,
        caption=f'<b><tg-emoji emoji-id="{EMOJI_DOWNLOAD}">⬇</tg-emoji> Экспорт базы данных</b>\n\n'
                f'Пользователей: {len(users)}\n'
                f'Аккаунтов: {len(accounts)}\n'
                f'Покупок: {len(purchases)}'
    )
    await callback.answer()

@dp.callback_query(F.data == "admin_broadcast")
async def broadcast_callback(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Нет доступа", show_alert=True)
        return
    
    await callback.message.edit_text(
        f'<b><tg-emoji emoji-id="{EMOJI_MEGAPHONE}">📣</tg-emoji> Отправьте сообщение для рассылки:</b>',
        reply_markup=get_broadcast_keyboard()
    )
    await state.set_state(BroadcastStates.waiting_for_message)
    await callback.answer()

@dp.message(StateFilter(BroadcastStates.waiting_for_message))
async def process_broadcast(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    
    p = await get_pool()
    async with p.acquire() as conn:
        users = await conn.fetch("SELECT user_id FROM users")
    
    msg = await message.answer(f'<tg-emoji emoji-id="{EMOJI_LOADING}">🔄</tg-emoji> Начинаю рассылку...')
    
    success = 0
    failed = 0
    
    for user in users:
        try:
            await message.copy_to(user[0])
            success += 1
            await asyncio.sleep(0.05)
        except:
            failed += 1
    
    await msg.edit_text(
        f'<b><tg-emoji emoji-id="{EMOJI_CHECK}">✅</tg-emoji> Рассылка завершена!</b>\n\n'
        f'<tg-emoji emoji-id="{EMOJI_CHECK}">✅</tg-emoji> Успешно: {success}\n'
        f'<tg-emoji emoji-id="{EMOJI_CROSS}">❌</tg-emoji> Неудачно: {failed}'
    )
    await state.clear()

# --- ПОЭТАПНОЕ ДОБАВЛЕНИЕ АККАУНТА ---
@dp.callback_query(F.data == "admin_add_acc")
async def admin_add_acc_start(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Нет доступа", show_alert=True)
        return
    
    await callback.message.edit_text(
        f'<b><tg-emoji emoji-id="{EMOJI_BOX}">📦</tg-emoji> Добавление аккаунта</b>\n\n'
        f'<b><tg-emoji emoji-id="{EMOJI_GEO}">📍</tg-emoji> Шаг 1/5: Введите страну аккаунта</b>\n\n'
        f'<tg-emoji emoji-id="{EMOJI_WRITE}">✍</tg-emoji> Пример: Россия, Украина, Казахстан',
        reply_markup=get_cancel_add_keyboard()
    )
    await state.set_state(AddAccountStates.waiting_for_country)
    await callback.answer()

@dp.message(StateFilter(AddAccountStates.waiting_for_country))
async def add_account_country(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    
    country = message.text.strip()
    if not country:
        await message.answer(
            f'<b><tg-emoji emoji-id="{EMOJI_CROSS}">❌</tg-emoji> Страна не может быть пустой!</b>\n\n'
            f'Введите страну ещё раз:',
            reply_markup=get_cancel_add_keyboard()
        )
        return
    
    await state.update_data(country=country)
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI_CHECK}">✅</tg-emoji> Страна: {country}</b>\n\n'
        f'<b><tg-emoji emoji-id="{EMOJI_MONEY}">🪙</tg-emoji> Шаг 2/5: Введите цену в рублях</b>\n\n'
        f'<tg-emoji emoji-id="{EMOJI_WRITE}">✍</tg-emoji> Пример: 500',
        reply_markup=get_cancel_add_keyboard()
    )
    await state.set_state(AddAccountStates.waiting_for_price)

@dp.message(StateFilter(AddAccountStates.waiting_for_price))
async def add_account_price(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    
    try:
        price = int(message.text.strip())
        if price <= 0:
            raise ValueError
    except ValueError:
        await message.answer(
            f'<b><tg-emoji emoji-id="{EMOJI_CROSS}">❌</tg-emoji> Введите корректную цену (целое число > 0)!</b>',
            reply_markup=get_cancel_add_keyboard()
        )
        return
    
    await state.update_data(price=price)
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI_CHECK}">✅</tg-emoji> Цена: {price}₽</b>\n\n'
        f'<b><tg-emoji emoji-id="{EMOJI_MEDIA}">🖼</tg-emoji> Шаг 3/5: Отправьте фото/видео аккаунта</b>\n\n'
        f'<tg-emoji emoji-id="{EMOJI_WRITE}">✍</tg-emoji> Или нажмите "Пропустить"',
        reply_markup=get_skip_media_keyboard()
    )
    await state.set_state(AddAccountStates.waiting_for_media)

@dp.message(StateFilter(AddAccountStates.waiting_for_media), F.photo | F.video | F.animation)
async def add_account_media(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    
    file_id, media_type = await get_media_file_id(message)
    
    await state.update_data(media_file_id=file_id, media_type=media_type)
    
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI_CHECK}">✅</tg-emoji> Медиа добавлено!</b>\n\n'
        f'<b><tg-emoji emoji-id="{EMOJI_PHONE}">📱</tg-emoji> Шаг 4/5: Введите номер телефона</b>\n\n'
        f'<tg-emoji emoji-id="{EMOJI_WRITE}">✍</tg-emoji> Пример: +79991234567',
        reply_markup=get_cancel_add_keyboard()
    )
    await state.set_state(AddAccountStates.waiting_for_phone)

@dp.callback_query(F.data == "skip_media", StateFilter(AddAccountStates.waiting_for_media))
async def skip_media(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        return
    
    await state.update_data(media_file_id=None, media_type=None)
    
    await callback.message.edit_text(
        f'<b><tg-emoji emoji-id="{EMOJI_PHONE}">📱</tg-emoji> Шаг 4/5: Введите номер телефона</b>\n\n'
        f'<tg-emoji emoji-id="{EMOJI_WRITE}">✍</tg-emoji> Пример: +79991234567',
        reply_markup=get_cancel_add_keyboard()
    )
    await state.set_state(AddAccountStates.waiting_for_phone)
    await callback.answer()

@dp.message(StateFilter(AddAccountStates.waiting_for_phone))
async def add_account_phone(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    
    phone = message.text.strip()
    if not re.match(r'^\+?[0-9]{10,15}$', phone.replace(' ', '').replace('-', '')):
        await message.answer(
            f'<b><tg-emoji emoji-id="{EMOJI_CROSS}">❌</tg-emoji> Неверный формат номера!</b>\n\n'
            f'Введите номер в формате +79991234567',
            reply_markup=get_cancel_add_keyboard()
        )
        return
    
    await state.update_data(phone=phone)
    
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI_CHECK}">✅</tg-emoji> Номер: {phone}</b>\n\n'
        f'<b><tg-emoji emoji-id="{EMOJI_LOCK_CLOSED}">🔒</tg-emoji> Шаг 5/5: Введите 2FA пароль</b>\n\n'
        f'<tg-emoji emoji-id="{EMOJI_WRITE}">✍</tg-emoji> Если 2FA нет, отправьте прочерк: -',
        reply_markup=get_cancel_add_keyboard()
    )
    await state.set_state(AddAccountStates.waiting_for_twofa)

@dp.message(StateFilter(AddAccountStates.waiting_for_twofa))
async def add_account_twofa(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    
    twofa = message.text.strip()
    if twofa == "-":
        twofa = None
    
    await state.update_data(twofa=twofa)
    data = await state.get_data()
    
    # Сохраняем аккаунт в БД
    p = await get_pool()
    async with p.acquire() as conn:
        await conn.execute(
            "INSERT INTO accounts (country, price, phone, twofa, media_file_id, media_type) VALUES ($1, $2, $3, $4, $5, $6)",
            data["country"], data["price"], data["phone"], 
            data.get("twofa"), data.get("media_file_id"), data.get("media_type")
        )
    
    twofa_text = f'\n<tg-emoji emoji-id="{EMOJI_LOCK_CLOSED}">🔒</tg-emoji> 2FA: {data.get("twofa")}' if data.get("twofa") else ''
    media_text = f'\n<tg-emoji emoji-id="{EMOJI_MEDIA}">🖼</tg-emoji> Медиа: добавлено' if data.get("media_file_id") else ''
    
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI_CHECK}">✅</tg-emoji> Аккаунт успешно добавлен!</b>\n\n'
        f'<tg-emoji emoji-id="{EMOJI_GEO}">📍</tg-emoji> Страна: {data["country"]}\n'
        f'<tg-emoji emoji-id="{EMOJI_MONEY}">🪙</tg-emoji> Цена: {data["price"]}₽\n'
        f'<tg-emoji emoji-id="{EMOJI_PHONE}">📱</tg-emoji> Номер: {data["phone"]}'
        f'{twofa_text}{media_text}',
        reply_markup=get_main_keyboard()
    )
    await state.clear()

@dp.callback_query(F.data == "cancel_add")
async def cancel_add(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        return
    
    await state.clear()
    await callback.message.edit_text(
        f'<b><tg-emoji emoji-id="{EMOJI_CROSS}">❌</tg-emoji> Добавление отменено</b>',
        reply_markup=get_back_button()
    )
    await callback.answer()

# --- ИЗМЕНЕНИЕ РЕКВИЗИТОВ ---
@dp.callback_query(F.data == "admin_change_req")
async def admin_change_req(callback: CallbackQuery):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Нет доступа", show_alert=True)
        return
    
    p = await get_pool()
    async with p.acquire() as conn:
        rows = await conn.fetch("SELECT key, value FROM requisites WHERE key IN ('sbp_phone', 'sbp_bank', 'sbp_fio')")
        reqs = {row['key']: row['value'] for row in rows}
    
    text = f'<b><tg-emoji emoji-id="{EMOJI_PENCIL}">🖋</tg-emoji> Текущие реквизиты СБП:</b>\n\n'
    text += f'<tg-emoji emoji-id="{EMOJI_PHONE}">📱</tg-emoji> Номер: <code>{reqs.get("sbp_phone", "не указан")}</code>\n'
    text += f'<tg-emoji emoji-id="{EMOJI_BANK}">🏦</tg-emoji> Банк: {reqs.get("sbp_bank", "не указан")}\n'
    text += f'<tg-emoji emoji-id="{EMOJI_PEOPLE}">👥</tg-emoji> ФИО: {reqs.get("sbp_fio", "не указано")}\n\n'
    text += f'<b><tg-emoji emoji-id="{EMOJI_WRITE}">✍</tg-emoji> Выберите, что хотите изменить:</b>'
    
    await callback.message.edit_text(text, reply_markup=get_requisites_keyboard())
    await callback.answer()

@dp.callback_query(F.data.startswith("edit_req_"))
async def edit_requisite(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("Нет доступа", show_alert=True)
        return
    
    key = callback.data.replace("edit_req_", "")
    names = {"sbp_phone": "номер телефона", "sbp_bank": "название банка", "sbp_fio": "ФИО получателя"}
    
    await state.update_data(edit_key=key)
    await state.set_state(ChangeRequisites.waiting_for_new_value)
    
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Отмена", callback_data="admin_change_req", style='danger', icon_custom_emoji_id=EMOJI_CROSS))
    
    await callback.message.edit_text(
        f'<b><tg-emoji emoji-id="{EMOJI_PENCIL}">🖋</tg-emoji> Введите новое значение для "{names[key]}":</b>',
        reply_markup=builder.as_markup()
    )
    await callback.answer()

@dp.message(StateFilter(ChangeRequisites.waiting_for_new_value))
async def save_requisite(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        return
    
    data = await state.get_data()
    key = data['edit_key']
    value = message.text.strip()
    
    if not value:
        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(text="Отмена", callback_data="admin_change_req", style='danger', icon_custom_emoji_id=EMOJI_CROSS))
        await message.answer(
            f'<b><tg-emoji emoji-id="{EMOJI_CROSS}">❌</tg-emoji> Значение не может быть пустым!</b>',
            reply_markup=builder.as_markup()
        )
        return
    
    p = await get_pool()
    async with p.acquire() as conn:
        await conn.execute("UPDATE requisites SET value = $1 WHERE key = $2", value, key)
    
    names = {"sbp_phone": "Номер", "sbp_bank": "Банк", "sbp_fio": "ФИО"}
    
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI_CHECK}">✅</tg-emoji> {names[key]} обновлён!</b>\n\n'
        f'Новое значение: <code>{value}</code>',
        reply_markup=get_main_keyboard()
    )
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI_SETTINGS}">⚙</tg-emoji> Админ-панель:</b>',
        reply_markup=get_admin_keyboard()
    )
    await state.clear()

@dp.callback_query(F.data == "back_to_admin")
async def back_to_admin(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        return
    
    await state.clear()
    await callback.message.edit_text(
        f'<b><tg-emoji emoji-id="{EMOJI_SETTINGS}">⚙</tg-emoji> Админ-панель:</b>',
        reply_markup=get_admin_keyboard()
    )
    await callback.answer()

@dp.callback_query(F.data == "close_panel")
async def close_panel(callback: CallbackQuery):
    await callback.message.delete()
    await callback.answer()

@dp.callback_query(F.data.in_({"cancel_payment", "cancel_selection"}))
async def cancel_action(callback: CallbackQuery):
    await callback.message.delete()
    await callback.answer("Отменено")

# --- ЗАПУСК ---
async def main():
    await init_db()
    os.makedirs("sessions", exist_ok=True)
    await dp.start_polling(bot)

if __name__ == "__main__":
    asyncio.run(main())
