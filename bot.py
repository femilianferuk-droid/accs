...
class LocalDB:
    ...
    def init_tables(self):
        with self.get_connection() as conn:
            ...
            # Вставляем дефолтные реквизиты если таблица пуста
    ...
    # Account methods
    def add_account(self, country: str, price: int, phone: str, code_hash: str, 
                    two_fa: str, session_string: str) -> int:
        with self.get_connection() as conn:
            cursor = conn.execute("""
                INSERT INTO accounts (country, price_rub, phone, code_hash, two_fa, session_string, status)
                VALUES (?, ?, ?, ?, ?, ?, 'available')
            """, (country, price, phone, code_hash, two_fa, session_string))
            conn.commit()
...
# Временное хранилище для процесса добавления аккаунта
...
def get_payment_method_keyboard(account_id: int, price: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text=f"🤖 Crypto Bot (~{usdt_price} USDT)",
...
@dp.message(F.text.in_(["🏠 Главная", "/menu"]))
...
async def profile(message: Message):
    ...
    if purchases:
        for p in purchases[:5]:
            purchases_text += f"• {p['country']} | {p['phone']} | {p['amount_rub']}₽\n"
    else:
...
<b><tg-emoji emoji-id="5884479287171485878">📦</tg-emoji> Последние покупки:</b>
{purchases_text}
"""
...
async def view_account(callback: CallbackQuery):
    ...
    if not account or account['status'] != 'available':
        await callback.answer("<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> Аккаунт недоступен", show_alert=True)
        return
...
"""
            ...
            await callback.answer("<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> Ошибка создания счета", show_alert=True)
...
async def check_crypto_payment(callback: CallbackQuery):
    ...
    if not purchase:
        await callback.answer("<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> Покупка не найдена", show_alert=True)
        return
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if result.get("ok") and result["result"]["items"]:
            ...
            if invoice["status"] == "paid":
                # Помечаем покупку как выполненную
...
"""
                ...
                await callback.answer("<tg-emoji emoji-id='5870633910337015697'>✅</tg-emoji> Оплата подтверждена!", show_alert=True)
            else:
                await callback.answer("<tg-emoji emoji-id='5983150113483134607'>⏰</tg-emoji> Оплата еще не получена", show_alert=True)
        else:
            await callback.answer("<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> Ошибка проверки", show_alert=True)
...
"""
    ...
    await callback.answer("<tg-emoji emoji-id='5870633910337015697'>✅</tg-emoji> Оплата одобрена!", show_alert=True)
...
"""
    ...
    await callback.answer("<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> Оплата отклонена", show_alert=True)
...
async def get_code(callback: CallbackQuery):
    ...
    if not account:
        await callback.answer("<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> Аккаунт не найден", show_alert=True)
        return
    
    await callback.answer("<tg-emoji emoji-id='5345906554510012647'>🔄</tg-emoji> Получаем код...")
    ...
    try:
        ...
        if not await client.is_user_authorized():
            await callback.message.answer("<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> Ошибка: сессия недействительна")
        ...
        # Ищем сервисные сообщения с кодом
        code_found = None
...
"""
            ...
            await callback.message.answer(
                "<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> Код не найден. Возможно, он уже был использован или истек."
            )
            
    except Exception as e:
        await callback.message.answer(f"<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> Ошибка: {str(e)}")
...
async def add_account_price(message: Message, state: FSMContext):
    try:
        ...
        await message.answer("<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> Введите корректное число")
        return
    
    prompt_text = """
<b><tg-emoji emoji-id="5771851822897566479">🔡</tg-emoji> Добавление нового аккаунта</b>

Введите API ID (можно получить на my.telegram.org):
"""
    await message.answer(prompt_text, reply_markup=get_back_button("admin_menu"))
    await state.set_state(AddAccountStates.waiting_for_api_id)

@dp.message(AddAccountStates.waiting_for_api_id)
async def add_account_api_id(message: Message, state: FSMContext):
    try:
        api_id = int(message.text)
        await state.update_data(api_id=api_id)
    except ValueError:
        await message.answer("<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> Введите корректный API ID")
        return
...
@dp.message(AddAccountStates.waiting_for_api_hash)
...
"""
        ...
        await message.answer(f"<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> Ошибка: {str(e)}")
        await state.clear()
        await client.disconnect()
...
async def add_account_code(message: Message, state: FSMContext):
    ...
    if not client:
        await message.answer("<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> Сессия истекла, начните заново")
        await state.clear()
        return
    ...
    try:
        ...
        try:
            if await client.is_user_authorized():
                # Проверяем, есть ли облачный пароль
                pass
...
Цена: {data['price']}₽
Телефон: {data['phone']}
2FA: {"установлен" if password else "отсутствует"}
""", reply_markup=get_admin_keyboard())
...
async def admin_sbp_settings(message: Message):
    if message.from_user.id not in ADMIN_IDS:
        return
...
@dp.callback_query(F.data == "edit_sbp")
...
"""
        try:
            ...
            await message.answer(f"<tg-emoji emoji-id='5870657884844462243'>❌</tg-emoji> Ошибка загрузки заявки #{purchase['id']}: {e}")
...
"""
    ...
    if callback.from_user.id in ADMIN_IDS:
        await callback.message.answer(menu_text, reply_markup=get_admin_keyboard())
    else:
...
async def admin_menu_callback(callback: CallbackQuery):
    ...
    await callback.message.delete()
    await callback.message.answer(
        "<b><tg-emoji emoji-id='5873147866364514353'>🏠</tg-emoji> Админ панель</b>",
        reply_markup=get_admin_keyboard()
    )
    await callback.answer()
