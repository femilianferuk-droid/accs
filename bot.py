...
def get_skip_media_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Пропустить", callback_data="skip_media", icon_custom_emoji_id=EMOJI_SEND)],
        [InlineKeyboardButton(text="Отмена", callback_data="cancel_add", icon_custom_emoji_id=EMOJI_CROSS)]
    ])
...
async def profile(message: Message):
    ...
    if purchases:
        # Отправляем медиа покупок
        for p in purchases[:5]:
            if p[2] and p[3]:  # media_file_id и media_type
                caption = f'<tg-emoji emoji-id="{EMOJI_LOCK_OPEN}">🔓</tg-emoji> {p[1]} - {p[4]}₽ ({p[5][:10]})'
                try:
                    if p[3] == "photo":
                ...
                await message.answer(f'<tg-emoji emoji-id="{EMOJI_LOCK_OPEN}">🔓</tg-emoji> {p[1]} - {p[4]}₽ ({p[5][:10]})')
...
async def select_account(callback: CallbackQuery):
    ...
    async with aiosqlite.connect("vest_accounts.db") as db:
        cursor = await db.execute(
            "SELECT country, price, media_file_id, media_type, description FROM accounts WHERE id = ? AND status = 'available'",
            (acc_id,)
        )
...
async def admin_export_db(callback: CallbackQuery):
    ...
    async with aiosqlite.connect("vest_accounts.db") as db:
        ...
        cursor = await db.execute("SELECT id, country, price, phone, code, twofa, status, created_at FROM accounts")
        accounts = await cursor.fetchall()
...
async def add_account_phone(message: Message, state: FSMContext):
    ...
    # Отправляем запрос кода через Telethon
...
async def add_account_twofa(message: Message, state: FSMContext):
    ...
    # Ждём немного чтобы код успел прийти
    ...
    async with aiosqlite.connect("vest_accounts.db") as db:
        await db.execute(
            "INSERT INTO accounts (country, price, phone, code, twofa, media_file_id, media_type) VALUES (?, ?, ?, ?, ?, ?, ?)",
